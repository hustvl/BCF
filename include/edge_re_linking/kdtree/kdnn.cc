...
#undef DEBUG_RUN_NN_SEARCH  //Follow the tree querying process
...
#include "kdtree_common_fixed.cc"//Nagesh Adluru. April 22, 2007.

void mexFunction( int nlhs, mxArray **plhs, int nrhs, const mxArray **prhs){
  Tree         *tree;
  double       *nn, *querypt, *tmppt;
  int          *index, i,j;
  unsigned int  N, M;
  ...
  if (nrhs <3 ) {
    mexErrMsgTxt("Must have at least 3 inputs (ROOT, QUERYPT, N).\n"); //, [MinNumberOfNeighbors].");
  }
  ...
  if ( (mxGetM(prhs[2]) != 1)  || (mxGetN(prhs[2])!=1))
    mexErrMsgTxt("Number of neighbors (N) must be a scalar value >= 0.");
  if ( nn[0] < 0 )
    mexErrMsgTxt("Number of neighbors (N) must be a scalar value >= 0.");


  if ( nn[0] > tree->numElements )
    mexErrMsgTxt("Number of Neighbors (N) must be less than size of tree.");
  ...
  if (!SkipQueries) {
    ...
    run_nn_search(tree->rootptr,querypt,M,tree->dims,nn[0],
		     &found_pts, &L, &indices);
