...
//   The following two functions are for the quicksort algorithm 
//
int nn_partition(unsigned int *a, int p, int r, double *dists){
  int i,j;
  double x;
  
  x= dists[a[p]]; i=p-1; j=r+1;
  while (1) {
    for (j--; dists[a[j]] > x; j--);
    for (i++; dists[a[i]] < x; i++);
    if (i<j){
...
void nn_quicksort(unsigned int *ra, int p, int r, double *dists){
  int q;
  
  if (p < r) {
    q = nn_partition(ra,p,r,dists);
    nn_quicksort(ra,p,q,dists);
    nn_quicksort(ra,q+1,r,dists);
  }
}

//
// The top-level function for running a n-nearest neighbor 
// search on the k-D tree.
//
void run_nn_search( Node *pVertex, double *model, int M, int D, 
		       int nn, double **pts_in_range, 
		       unsigned int *L, unsigned int **indices){
  ...
  double *pt;
  Node *LL, *cur, *leaf, *tmp;
  double *pts_found, *distances;
  unsigned int *dist_indices, *pts_indices;
  int k;

  float distlim=.000001;

  pt= (double*)malloc(sizeof(double)*D);
  for (i=0; i < M; i++) {  // Only called with M=1

#ifdef DEBUG_RUN_NN_SEARCH
    mexPrintf("Running NN Search (%d/%d) (value: ",
	      i+1, M);
    for (j=0; j < D ; j++)
      mexPrintf(" %f", model[ M*j+i]);
    mexPrintf(" )\n");
    ...
    *L=count;
    pts_found =  (double*)malloc(sizeof(double)*count*D);
    pts_indices =  (unsigned int*)malloc(sizeof(unsigned int)*count);
    cur=LL;
    count=0;
    while(cur){
      pts_indices[count] = cur->index;
      for (j=0; j < D; j++) {
	pts_found[j*(*L)+count] = cur->pt[j];
      }
      tmp=cur;
      cur=cur->right;
      free(tmp->pt);
    ...
    //Do quicksort of distances
    distances = (double*)malloc(sizeof(double)*count);
    for (k=0; k < count; k++) {
      distances[k]=0;
      for (j=0; j<D; j++)
	distances[k] += (pts_found[j*(*L)+k]-pt[j]) *
	  (pts_found[j*(*L)+k]-pt[j]);
    }
    
    dist_indices = (unsigned int*)malloc(sizeof(unsigned int)*count);
    for (k=0; k < count; k++) dist_indices[k]=k;

    nn_quicksort(dist_indices,0,count-1,distances);

    *L=nn;
    (*indices) = (unsigned int *) malloc (sizeof(unsigned int)*nn);
    (*pts_in_range) = (double *) malloc (sizeof(double) *nn*D);
    for (k=0; k < nn; k++) {
      (*indices)[k] =  pts_indices[dist_indices[k]];
      for (j=0; j <D; j++) 
	(*pts_in_range)[j*(*L)+k] = 
	  pts_found[j*(count)+dist_indices[k]];
    }
